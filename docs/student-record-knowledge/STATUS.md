...
- observation board 2 tab: `/observation-board-2`에서 예시 PNG에 맞춘 독립형 교실 대시보드, 기본 `멘토 멘티 활동 기록` 활동판, 모둠 추가, 차시별 활동 기록 표, 참여/매우 잘함 클릭 표시 지원
- observation board 2 internal dashboards: 왼쪽 메뉴가 URL 이동 없이 `홈`, `멘토 멘티 활동 기록`, `성장 기록`, `통계 보기`, `알림장`, `설정` 화면을 전환하며, 성장 기록/통계/알림장/설정 대시보드 구현
...
- 2026-04-25: renamed the default observation-board-2 screen to `멘토 멘티 활동 기록`, removed unused top-right action buttons, added group creation, restricted displays to assigned class students, and made growth/stats show actionable student and group signals
