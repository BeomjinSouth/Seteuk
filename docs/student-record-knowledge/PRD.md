...
- `/write`: default seteuk generation prompt uses the `cross-curricular-seteuk-v2` teacher-review draft policy; it prioritizes direct teacher observations and student-specific evidence, adjusts length and competency wording to the amount of evidence, improves sentence flow without fabricating facts, and excludes score/rank/award/test-item/student-level/unsupported subject-concept/AI-tool wording
- `/settings/ai`: teachers choose exactly one of `기본 설정` and `내 프롬프트`; the default is read-only `cross-curricular-seteuk-v2`, while the personal prompt is teacher-private and synced in workspace state
...
- AI settings stores admin overrides in browser storage, while empty or legacy default values resolve back to the shared `cross-curricular-seteuk-v2` default prompt; legacy default example templates are migrated to the new neutral observation-based example while teacher-edited templates are preserved
