...
export const POST = withTeacherAuth(async (request) => {
    ...
    const curriculumContextText = formatCurriculumContextForPrompt(curriculumContext);
    ...
    if (curriculumContextText) {
        userPrompt += `\n\n[선택 단원 context]\n${curriculumContextText}\n\n// 선택 단원 context는 수업 배경입니다. 학생 성취 근거가 아닙니다.\n// 학생별 관찰 메모, 학습 데이터, 수행 결과에 직접 드러난 행동이 있을 때만 단원 개념과 연결하세요.\n// 단원명이나 개념만 보고 이해도, 역량, 태도, 리더십을 새로 만들지 마세요.\n// 최종 문장에는 횟수나 차시 수를 그대로 쓰지 말고 관찰 가능한 행동 표현으로 바꾸세요.`;
    }

    // Add curriculum content if provided
    if (!curriculumContextText && curriculumContent && curriculumContent.trim()) {
...
function buildFallbackContext(
    ...
    const selectedUnit = curriculumContext?.selectedUnits?.[0];
    if (selectedUnit) {
        const concepts = selectedUnit.concepts.slice(0, 3).join(', ');
        return `${gradePrefix}${subject} 교과의 ${selectedUnit.unit}${concepts ? ` (${concepts})` : ''} 맥락`;
    }
