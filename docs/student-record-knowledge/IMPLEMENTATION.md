...
- `/observation-board-2` defaults to the PNG-like `멘토 멘티 활동 기록` screen and removes the earlier top eyebrow, internal mode tabs, class chips, search bar, and unused header action buttons from that first mentor/mentee view
- `/observation-board-2` manages internal board modes (`home`, `mentor`, `growth`, `stats`, `notice`, `settings`, `records`) without leaving the route; the visible sidebar exposes only `홈`, `멘토 멘티 활동 기록`, `성장 기록`, `통계 보기`, `알림장`, and `설정`
