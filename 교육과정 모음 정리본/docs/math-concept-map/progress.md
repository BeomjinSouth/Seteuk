...
- cross-band edge는 문서 직접 서술 기반 16개로 시작했다. 성취수준 문서·연구보고서에서 추가 연계 서술을 확인하면 같은 규칙으로 늘리고, 교과서 확보 후에는 교과서 근거 연결도 추가한다.
- 교과서 PDF가 확보되면 초등·고1 데이터셋에도 중학교와 같은 교과서 근거 패킷 체계를 적용하고, 미추출 수식 기호(초1-2 +,−,=,<,>; 공통수학 공식 목록)를 시각 확인해 반영한다.
