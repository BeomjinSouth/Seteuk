...
- `동위각과 엇각 위치 혼동`, `꼬인 위치와 평행 혼동`, `호와 현 혼동`, `겉넓이와 부피 혼동`, `관찰 결과와 증명 혼동`, `외심과 내심 혼동`, `이등변삼각형의 밑각과 꼭지각 혼동`, `사각형 포함 관계를 반대로 이해하는 오류`, `합동과 닮음 혼동`, `삼각비 범위 혼동`, `접선과 반지름 관계 오류` 등 도형 영역의 오개념 위험도 공식 문서와 성취수준에 근거한 잠정 노드이며, 교과서 예제·오답·문항 근거를 추가해야 한다.
...
- `review-queue.md`와 `review-queue.csv`는 위 잠정 노드 중 `confidence: low`인 99개 concept을 다음 교과서·출처 보강 우선순위로 모아 둔 파생 산출물이다.
...
- `relationship-audit.md`와 `relationship-audit.csv`는 현재 edge 3090개가 관계 유형별로 어떻게 분포하는지와 고립 concept이 없는지 추적하는 파생 산출물이다.
...
- 현재 연구보고서 신호는 263개 row로 잡혔으며, 이 산출물은 후보 신호이므로 같은 concept이 여러 page 맥락에 등장할 수 있다.
...
- `research-report-context-packet.md`와 `research-report-context-packet.csv`를 추가해 low-confidence 후보와 상위 medium 후보를 연구보고서 page 맥락 67개 row로 좁혔다.
...
- `research-report-source-review.md`와 `research-report-source-review.csv`는 같은 67개 row를 `candidate_prerequisite_evidence` 3개, `candidate_assessment_item_evidence` 8개, `candidate_achievement_level_evidence` 15개, `broad_report_context_only` 38개, `weak_occurrence_only` 3개로 분리한다.
- `비`의 직접 연구보고서 후보 page 61, 172, 180, 181, 184, `사건 A 또는 사건 B가 일어나는 경우의 수`의 page 228, 240, `반비례`와 `교점`의 page 58, `전개도`의 page 173, 174, `기둥 모양 입체도형`의 page 103, 108, `뿔 모양 입체도형`의 page 174, 181, `평균`의 page 177, `삼각형`·`길이`·`넓이`의 page 62 도형 평가 과제 맥락, `대각선`과 `한 변의 길이가 1인 정사각형의 대각선`의 page 213 무리수 성취수준 맥락은 source ref로 수동 적용되어 `applied_after_manual_review` 18개로 표시된다. `비율그래프`만으로 매칭된 page 183은 `비` concept의 직접 근거로 보지 않아 제외했고, 자료·좌표·수식을 도구에 `입력`하는 연구보고서 page는 함수의 `입력값` 직접 근거로 보지 않아 제외했다. 남은 row는 `not_applicable_from_this_row` 41개, `pending_manual_review` 8개이며, 새 pending row는 주로 `선분`·`반직선` 연구보고서 후보라 교과서 본문 또는 직접 중학교 기본 도형 근거 확인 전까지 자동 반영하지 않는다. 정수와 유리수 보강에서는 연구보고서 p. 211의 음수·양수·정수·유리수 성취수준 맥락과 p. 212의 대소 관계·수직선·사칙계산 성취수준 맥락을 직접 확인해 보조 source ref로 반영했지만, 이 페이지들은 기존 context packet에서 새로 승격된 항목이 아니므로 source-review 적용 개수에는 더하지 않는다. 이번 이차함수 보강에서는 연구보고서 p. 220의 이차함수 성취수준 맥락을 직접 확인해 `이차함수 판별하기`, `상황을 이차함수 식으로 나타내기`, `y=ax^2`, `y=a(x-p)^2`, `y=ax^2+q`, `y=a(x-p)^2+q`, `y=ax^2+bx+c`, 값의 표와 그래프 성질 읽기 보조 source ref로 반영했다. 이번 도수분포표와 상대도수 보강에서는 연구보고서 p. 227~228의 성취수준 맥락을 직접 확인해 도수분포표·히스토그램·도수분포다각형으로 나타내기, 상대도수 구하기, 상대도수의 분포를 표나 그래프로 나타내고 해석하기, 통계적 탐구 문제 설정과 공학 도구 활용의 보조 source ref로 반영했다. 이번 기본 도형 보강에서는 연구보고서 p. 221 후보를 확인했지만, 공식 교육과정·성취수준 문서만으로 충분히 지지되는 미시 수행 노드에 우선 반영하고 p. 221은 교과서 근거를 대체하지 않는 후보 신호로만 남겼다. 이번 소인수분해 보강에서는 연구보고서 row를 새 source ref로 붙이지 않고 교육과정의 수와 연산 성취기준, 용어·기호, 교수학습 유의사항, 성취수준 문서만 사용했다. 특히 다항식 인수분해 근거인 연구보고서 p. 219는 소인수분해 근거로 전용하지 않고, `소인수`와 대수적 `인수`의 구별 관계에만 제한적으로 참고한다. p. 220, p. 221, p. 227~228은 교과서 본문 근거가 아니므로 오개념 노드 confidence 승격이나 교과서 근거 대체에는 쓰지 않는다. 앞선 경우의 수와 확률 보강에서 직접 확인한 연구보고서 p. 260의 확률 평가 문항과 p. 266~268의 이동 경로 평가 문항도 같은 이유로 source-review 적용 개수에는 더하지 않는다. 교과서 또는 중학교 과정 직접 근거가 확인되기 전까지 `비`와 `입력값`, `선분`, `반직선`, `한 평면 위에 있음`, 새 확률값·0/1 사건, `절댓값이 같고 부호가 다른 두 수`, `뺄셈을 반대 부호의 덧셈으로 바꾸기`, `상대도수의 합`, `소인수분해의 유일성` 등 교과서 표현 확인이 필요한 미시 concept은 낮은 신뢰도를 유지한다.
...
- 이번 이차방정식 보강에서는 연구보고서 p. 220의 `[9수02-20]` 성취기준별 성취수준 맥락을 직접 확인해 `이차방정식`, `이차항`, `이차방정식 풀기`, `이차항의 계수가 1인 이차방정식`, `이차방정식 풀이 과정 설명하기`, `문제 상황을 이차방정식으로 나타내기`, `이차방정식 활용 문제 해결`의 보조 source ref로 반영했다. 같은 p. 220이라도 `근의 공식`, `중근`, `이차방정식의 실수 해 범위`, `근과 계수와의 관계 제외`, 오개념 노드의 직접 근거로는 쓰지 않고, 해당 항목은 교육과정 용어·범위 제한 근거와 추후 교과서 본문 확인 대상으로 유지했다. p. 220은 교과서 본문 근거가 아니므로 오개념 노드 confidence 승격이나 교과서 근거 대체에는 쓰지 않는다.
...
- 이번 삼각형과 사각형의 성질 보강에서는 공식 교육과정과 성취수준의 `[9수03-09]`~`[9수03-11]`, 도형과 측정 용어·기호, 증명 도입 유의사항을 근거로 이등변삼각형의 같은 변·밑변·꼭지각·밑각, 직각삼각형의 합동 조건, 외심과 세 변의 수직이등분선, 내심과 세 내각의 이등분선, 평행사변형·직사각형·마름모·정사각형·사다리꼴의 성질과 포함 관계를 보강했다. 교과서 본문 근거가 아직 없으므로 `이등변삼각형의 밑각과 꼭지각을 혼동하는 오류`, `사각형 포함 관계를 반대로 이해하는 오류`는 `confidence: low`와 `often_confused_with` 관계로만 유지하고, `관찰 결과와 증명 혼동`, `외심과 내심 혼동`의 noisy prerequisite도 제거했다.

## Source Reference Audit

- `source-ref-audit.md`와 `source-ref-audit.csv`를 추가해 concept/edge의 `source_refs`를 출처와 근거 유형별로 추적한다.
- 현재 source ref는 concept 1923개, edge 8041개, 총 9964개이다.
...
- 현재 694개 concept 중 `official_dual_source`는 595개, `official_single_source`는 99개이다.
- 현재 `textbook_supported` concept은 0개이며, 694개 concept 모두 교과서 본문·정리·예제·문제 근거 보강이 필요하다.
...
- 현재 3090개 edge 중 `official_dual_source`는 2700개, `official_single_source`는 390개이다.
- 현재 `textbook_supported` edge는 0개이며, 3090개 edge 모두 교과서 본문·예제·문제 근거 보강이 필요하다.
- 현재 edge scope 분포는 `same_unit` 2500개, `cross_unit_same_domain` 251개, `cross_domain_same_grade` 62개, `cross_grade_same_domain` 245개, `cross_grade_cross_domain` 32개이다.
...
- 현재 큐는 34개 단원 그룹을 포함하며, 교과서 근거 보강 필요 concept 694개와 `low` 신뢰도 concept 99개를 반영한다.
...
- 전체 패킷은 concept 694개를 포함하며, 그중 `low` 신뢰도 concept 99개를 단원 안에서 먼저 정렬한다.
...
- 현재 `교과서_원본/`에는 PDF가 없으므로 694개 row 모두 `pending_textbook_pdf` 상태이며, 목차·학습목표·정의·정리·예제·용어 설명·문제 패턴·쪽수 슬롯은 비워 두었다.
...
- `rank-01`~`rank-34` 패킷은 `textbook-extraction-queue.csv`의 전체 34개 단원 그룹을 포함한다.
- 전체 edge 패킷은 단원 concept에 닿는 관계 edge 3680개 row를 포함하며, 같은 edge가 cross-unit 관계이면 양쪽 단원 패킷에 중복 배치될 수 있다.
- 현재 분포는 intra-unit edge row 2500개, cross-unit edge row 1180개, `low` 신뢰도 edge row 528개이다.
- 각 row는 관계 유형별로 구조 근거, 선수 순서 근거, 표현 근거, 절차 활용 근거, 대조 근거, 오개념·문제 패턴 근거, 관련 근거, 쪽수 슬롯을 분리한다.
- 현재 `교과서_원본/`에는 PDF가 없으므로 3680개 row 모두 `pending_textbook_pdf` 상태이며, 관계 근거 슬롯은 비워 두었다.
...
- 전체 workplan은 34개 단원 그룹, concept evidence row 694개, edge evidence row 3680개, pending textbook evidence row 4374개를 포함한다.
- 전체 low-confidence 보강 대상은 concept 99개와 edge row 528개를 합친 627개 row이다.
- 현재 최상위 단원 `좌표평면과 그래프`는 concept 43개와 edge row 241개, 총 284개 row가 모두 `pending_textbook_pdf` 상태이다. workplan 재정렬 후 `이차함수와 그 그래프`는 rank 4이며 concept 30개와 edge row 143개, 총 173개 row가 모두 `pending_textbook_pdf` 상태이고, 이번 보강 대상인 `삼각형과 사각형의 성질`은 rank 6이며 concept 47개와 edge row 215개, 총 262개 row가 모두 `pending_textbook_pdf` 상태이다. `도수분포표와 상대도수`는 rank 7이며 concept 33개와 edge row 180개, 총 213개 row가 모두 `pending_textbook_pdf` 상태이고, `이차방정식`은 rank 8이며 concept 22개와 edge row 103개, 총 125개 row가 모두 `pending_textbook_pdf` 상태이다.
