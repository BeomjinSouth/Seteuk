...
async function syncStatus() {
  ...
  const body = [
    ...
    '- observation board 2 internal dashboards: 왼쪽 메뉴가 URL 이동 없이 `학생 관찰 기록`, `성장 기록`, `통계 보기` 화면만 전환하며, `홈`, `알림장`, `설정` 항목은 현재 사이드바에서 숨기고 헤더에서도 `알림` 진입점을 제거함',
    ...
    '- observation board 2 header actions: 동작 없는 `우정 배지`/`알림` 버튼을 제거하고, hydration 전에는 중립적인 `계정`으로 보이다가 로그인 상태가 붙으면 실제 교사 이름을 표시하는 계정 메뉴만 유지함',
    ...
    '- 2026-05-17: removed the observation-board-2 dead `우정 배지` and `알림` header buttons, kept the decorative cloud, and changed the teacher chip into a real account menu with school info, AI seteuk navigation, and logout actions',
    '- 2026-05-17: deployed the observation-board-2 header cleanup to Vercel production (`dpl_FeuncEQX4TeAbKmaBCpjky3hvJzQ`) after confirming the previous live deployment still contained the removed header buttons; live `/observation-board-2` now returns 200 without `우정 배지`, top `알림`, or `선생님` fallback in the prerendered response',
    '- 2026-05-17: changed the teacher chip fallback from `선생님` to neutral `계정`/`계정 확인 중`; after client hydration the chip displays the logged-in teacher name from workspace state',
    '- 2026-05-17: verification passed for `cmd /c npx tsc --noEmit --pretty false`, HTTP 200 route smoke on `http://localhost:3005/observation-board-2`, and Browser DOM/interaction checks proving the removed buttons are absent, the teacher menu opens, `학교 정보`/`AI 세특 생성` navigate, and `로그아웃` returns to `/`; Browser screenshot capture timed out, so visual proof used DOM snapshots instead',
