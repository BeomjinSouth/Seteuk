...
위 예시의 어휘, 어투, 표현 방식 등을 참고하여 작성해 주세요.`;
    ...
    if (!hasOpenAIApiKey()) {
        ...
        return NextResponse.json({
            success: true,
            content: generateFallbackContent(studentName, subjectName, sanitizedLearningData),
            fallback: true,
    ...
    try {
        ...
        const content = sanitizeGeneratedSeteukContent(response.output_text || '', sanitizedLearningData);
