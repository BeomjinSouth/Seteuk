...
- Graph RAG counsel mode: `/counsel-chat` now includes a Graph RAG tab with `/api/counsel-chat/graph`, answer-first citation annotations, highlighted answer spans only when a direct public source match exists, right-side source excerpt viewer, answer font-size slider, and supplementary ontology/data-flow graph visualization
