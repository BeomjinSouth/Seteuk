...
## 2026-07-03 상자그림과 산점도 미시 concept 빠른 추가 보강

- AGENTS.md를 다시 확인하고, 이번 작업도 PDF 원본이나 다운로드 manifest를 변경하지 않는 `docs/math-concept-map/` 생성 로직과 파생 산출물 정비로 제한했다.
- 사용자의 속도 피드백을 반영해 멀티에이전트 explorer 2개를 읽기 전용으로 사용했다. Jason은 기존 `상자그림과 산점도` 노드·edge와 누락 후보를, Lovelace는 `[9수04-08]`, `[9수04-09]`, 용어표, 교수·학습 유의사항, 성취수준 근거의 confidence boundary와 제외할 고급 통계 개념을 점검했다.
- `자료를 네 부분으로 나누는 세 값`, `아래쪽 절반의 자료`, `위쪽 절반의 자료`를 추가해 사분위수 계산에서 학생이 실제로 수행하는 자료 분할 판단을 미시 concept으로 분리했다.
- `상자그림의 중앙값 선`, `상자그림의 네 구간`, `상자그림 구간별 자료 비율`을 추가해 상자그림의 시각 요소와 구간별 25% 해석을 분리했다. 교과서 그림 설명 확인 전까지 구간·비율 해석은 `confidence: low`로 유지했다.
- `상자그림에서 중앙값 비교하기`, `상자 길이로 사분위범위 비교하기`, `수염 길이로 양쪽 퍼짐 비교하기`, `같은 눈금에서 두 상자그림 비교하기`를 추가해 “두 집단의 분포 비교”를 중심, 가운데 50% 퍼짐, 양쪽 퍼짐, 척도 확인 절차로 나누었다.
- `두 변량의 대응표`, `산점도 축 이름 확인하기`, `산점도 한 점의 의미 해석`, `오른쪽 위로 향하는 산점도 경향`, `오른쪽 아래로 향하는 산점도 경향`, `뚜렷한 경향이 없는 산점도 모양`, `직선에 가까운 산점도 경향`, `상관관계의 강하고 약한 정도`를 추가해 산점도 작성·해석을 표, 축, 점, 전체 경향, 정성적 강약 판단으로 세분화했다.
- `상자그림의 중앙값 선이 항상 상자의 가운데에 있다고 보는 오류`, `한 점만 보고 상관관계를 판단하는 오류`를 오개념 risk로 추가했다. 오개념 노드는 모두 `confidence: low`, 선수 관계 없음, `often_confused_with` 중심으로만 연결했다.
- 상관계수, 회귀직선, 공분산, 잔차, 1.5 IQR 이상치 규칙, 수정 상자그림, 사분위수 알고리즘 변형, 상자그림 구간 비율과 도수분포의 직접 prerequisite, 상관관계 강약과 기울기의 직접 prerequisite는 이번 중학교 범위에서 제외했다.
- 전체 산출물을 재생성한 결과 concept은 947개, edge는 4394개가 되었다. source ref audit은 14001개 ref를 기록하고 source catalogue는 5개를 유지한다.
- `review-queue.*`는 188개 low-confidence concept, `concept-evidence-depth.*`는 concept 947개, `edge-evidence-depth.*`는 edge 4394개, `prerequisite-map.*`은 1609개 선수 관계 edge로 갱신했다.
- `unit-coverage.*` 기준 `상자그림과 산점도`는 concept 52개, 내부 edge 242개, incoming edge 39개, outgoing edge 4개이며, confidence 분포는 high 12개, medium 15개, low 25개이다. concept type 분포는 core 2개, sub_concept 11개, representation 12개, procedure 13개, property 1개, term 7개, misconception_risk 6개이다.
- `textbook-evidence-workplan.*`에서 `상자그림과 산점도`는 rank 1, priority tier `highest`이며 concept 52개, edge 285개, 총 337개 evidence row가 모두 `pending_textbook_pdf` 상태다. 이 단원의 low-confidence concept은 25개, low-confidence edge는 125개이다.
- `test_build_pilot_data_box_scatter_microconcepts.py`를 확장해 사분위수 자료 분할, 상자그림 구성 요소와 구간 해석, 같은 눈금 비교, 두 변량 대응표, 산점도 점·축·경향·상관관계 강약 해석, 오개념 risk, noisy edge 부재를 고정했다.
- 좁은 테스트: `python -X utf8 -m unittest test_build_pilot_data_box_scatter_microconcepts -b`를 `docs/math-concept-map/tools`에서 실행해 5개 통과.
- 전체 validator: `python -X utf8 docs/math-concept-map/tools/validate_concept_map.py`: 947개 concept, 4394개 edge, 5개 source, 60개 공식 성취기준 검증 통과.
- 전체 단위 테스트: `python -X utf8 -m unittest discover -s docs/math-concept-map/tools -p "test_*.py" -b`: 334개 통과.
- 전체 직접 파생 산출물과 research-report/legacy 보조 산출물을 재생성했다. 교과서 evidence packet과 edge packet을 만든 뒤 `textbook-evidence-workplan.*`, 루트 `pilot-unit-map.*`, 전체 `unit-map-packets/*`를 다시 생성해 rank/count가 최신 packet index와 일치하도록 했다.
- `node-edge-consistency-audit.*`와 `related-edge-resolution-queue.*`는 모두 0건이다.
- PDF 원본과 `2022_개정_중학교_교육과정_PDF/DOWNLOAD_MANIFEST.md`, `2022_개정_중학교_성취수준_PDF/DOWNLOAD_MANIFEST.md`는 변경하지 않았다.
...
## 2026-07-07 초1-2 전체 영역 미시 concept 완성

- 사용자 지시(공통수학1·2까지 미시 분해 완주)에 따라 초1-2의 나머지 3개 영역을 분해했다. 데이터 구조를 밴드별 모듈로 재편해 `elementary_common.py`(공통 도우미)와 `elementary_data_e12_rest.py`(변화와 관계·도형과 측정·자료와 가능성)를 추가했고, `build_elementary_pilot.py`가 수와 연산 데이터와 병합해 같은 산출물을 생성한다.
- 추가 분해 근거는 별책8 인쇄 p.13~16의 성취기준([2수02-01]~[2수04-03]) 진술, 해설, 적용 시 고려 사항, 용어·기호 목록(삼각형, 사각형, 원, 꼭짓점, 변, 시, 분, 약, cm, m / 표, 그래프)이다.
- 초1-2 전체는 concept 119개, edge 114개가 되었다. 영역을 가로지르는 공식 근거 edge(곱셈표→수 배열표 규칙 찾기, 덧셈·뺄셈→길이의 덧셈과 뺄셈, 수 세기→분류한 개수 세기)를 포함한다.
- `test_build_elementary_pilot.py`를 4개 영역 범위와 교차 영역 edge 검증으로 확장했다(10개). 전체 단위 테스트 367개 통과.

## 2026-07-07 초3-4 전 영역 미시 concept 분해

- `elementary_data_e34.py`를 추가해 초3-4 전체 4개 영역(성취기준 [4수01-01]~[4수04-03] 47개, 별책8 인쇄 p.17~24)을 분해했다. 큰 수·나눗셈·분수·소수 도입, 도형의 기초(직선·각·수직·평행), 평면도형의 이동, 원의 구성 요소, 삼각형·사각형 분류, 다각형, 시각(초)·길이(mm·km)·들이(L·mL)·무게(g·kg·t)·각도(도) 측정, 그림·막대·꺾은선그래프와 통계적 탐구 과정을 미시 concept으로 나눴다.
- 용어·기호 목록(나눗셈, 몫, 나머지, 나누어떨어진다, 분수 관련 7개, 자연수, 소수, 소수점(.), ÷ / 도형·측정 38개 / 그래프 3개)을 term 노드 또는 기존 노드의 근거로 반영했다. ÷와 소수점(.)은 텍스트 추출로 직접 확인했다.
- 학년군 사이 확장 관계(예: 두 자리 수 계산→세 자리 수 계산, cm·m→mm·km, ○×/ 그래프→그림·막대그래프)는 `medium`+추론 메모로, 진술·해설·고려 사항의 직접 연결(예: 분모 10인 진분수→소수 한 자리 수, 등호의 의미→동치 관계)은 `high`로 연결했다.
- 초등 데이터셋은 초1-2와 합쳐 concept 240개, edge 243개가 되었다. 테스트는 초1-2/초3-4 두 학년군의 4개 영역 커버리지를 검증하도록 확장했다.

## 2026-07-07 초5-6 전 영역 미시 concept 분해 (초등 완성)

- `elementary_data_e56.py`를 추가해 초5-6 전체 4개 영역(성취기준 [6수01-01]~[6수04-06] 45개, 별책8 인쇄 p.25~32)을 분해했다. 혼합 계산, 수의 범위와 올림·버림·반올림, 약수와 배수, 분수·소수의 사칙연산, 대응 관계, 비와 비율·비례식·비례배분, 합동과 대칭, 입체도형(직육면체~구, 겨냥도·전개도, 공간 감각), 둘레·넓이·원주율·겉넓이·부피, 평균, 띠·원그래프, 가능성을 미시 concept으로 나눴다.
- 용어·기호 목록(이상~기약분수 16개 / 비~% 9개 / 합동~m³ 33개 / 평균·띠그래프·원그래프·가능성 4개)을 term 노드 또는 기존 노드 근거로 반영했다. :와 %는 텍스트 추출로 직접 확인했다.
- 학년군 확장 관계는 `medium`+추론 메모, 문서 직접 연결(예: 통분→분모 다른 분수 덧뺄, (원주)÷(지름)→원주율, 직사각형 넓이→원의 넓이 방법)은 `high`로 연결했다.
- 초등 데이터셋 전체는 concept 330개, edge 345개가 되었다. 초등 세 학년군 × 4개 영역 커버리지 테스트 통과.

## 2026-07-07 공통수학1·2 미시 concept 분해 (초1~공통수학2 미시 분해 완주)

- `build_hs_common_pilot.py`와 `test_build_hs_common_pilot.py`를 추가해 고등학교 공통 과목 공통수학1([10공수1-*] 19개 성취기준)과 공통수학2([10공수2-*] 20개 성취기준, 별책8 인쇄 p.57~67)를 분해했다. 산출물은 `hs-common-concepts.json/.csv`, `hs-common-edges.csv`, `hs-common-pilot.md`이며 concept 61개(공통수학1 34, 공통수학2 27), edge 56개다.
- 공통수학1은 다항식(나머지정리·인수정리·조립제법·미정계수법·인수분해), 방정식과 부등식(복소수·판별식·근과 계수·이차함수 연결·삼차/사차·연립이차·부등식), 경우의 수(합·곱의 법칙, 순열, 조합), 행렬(표현·연산)로, 공통수학2는 도형의 방정식(거리·내분·직선 조건·원·이동), 집합과 명제(집합·연산·명제·증명법·절대부등식), 함수와 그래프(대응 함수·합성·역함수·유리/무리함수)로 나눴다.
- 해설·고려 사항의 직접 연결(내분 전 두 점 사이 거리, 인수분해·인수정리·조립제법→삼차/사차방정식, 집합→대응 함수, 복소수 범위의 근)은 `high`, 배열 순서 추론은 `medium`+메모로 구분했다. 중학교 연계 서술(다항식·실수 연산·연립일차방정식·경우의 수·일차함수 그래프·함수 확장)은 notes와 md에 남겨 다음 단계의 중학교 미시 map 연결에 쓴다.
- 수식 글꼴(HyhwpEQ) 미추출 한계(인수분해 공식 목록, 유리·무리함수 기본형, y=x 표기)를 노드 notes와 md에 기록했다. 기본수학1·2는 공통수학의 대체 경로이므로 분해에서 제외하고 그 사실을 md에 남겼다.
- 이로써 사용자 목표였던 초1~공통수학2 전 구간 미시 분해 1차가 완료되었다: 초등 330개(초1-2 119, 초3-4 121, 초5-6 90) + 공통수학1·2 61개 = 신규 미시 concept 391개, edge 401개. 중학교는 기존 `concepts.json` 977개가 담당한다.
- 전체 단위 테스트: `python -X utf8 -m unittest discover -s docs/math-concept-map/tools -p "test_*.py" -b`: 374개 통과. validator: 977개 concept, 4571개 edge 통과(중학교 데이터 무변경).

## 2026-07-07 시각화 드릴다운을 미시 concept으로 확장

- `build_k12_spine_viz.py`가 세 미시 데이터셋(`elementary-concepts.json`, `concepts.json`, `hs-common-concepts.json`)을 로드해 각 블록에 미시 concept을 단원별로 묶어 붙이도록 확장했다. 매칭 규칙: 초등 domain 블록은 (grade, domain), 중1-3 domain 블록은 domain(중학교 concept 전체), 고1 area 블록은 (subject, unit)이다. 기본수학1·2(대체 경로)는 미시 매칭 대상에서 제외한다.
- `k12-spine.html`은 이제 블록 클릭 시 오른쪽 패널에 두 탭을 보인다: `미시 개념`(단원별 그룹, concept_type 배지와 위계 정렬, 짧은 정의, low-confidence `●` 표시)과 `성취기준`(원문·페이지 locator). 블록 부제도 `성취기준 N · 개념 M`으로 갱신된다. 전체 임베드 미시 concept 1368개(초등 330 + 중학교 977 + 고1 61), 파일 328KB.
- 미시 목록은 단원 안에서 핵심→하위→성질→표현→절차→용어→오개념 순으로 정렬해 위계가 드러나게 했다.
- 브라우저에서 초(초3-4 도형과 측정: 65개, 11개 단원), 중(중1-3 수와 연산: 125개, 6개 단원, 성취기준 탭 전환 시 [9수01-01]…), 고(공통수학1 다항식: 7개)와 자료와 가능성의 low-confidence `●` 69개 표시를 실제 클릭·탭 전환으로 검증했다. 콘솔 에러 없음.
- `test_build_k12_spine_viz.py`에 미시 concept 총계(1368) 일치, 스코프 블록의 미시 부착, 대체 경로 미부착, 단원 내 위계 정렬을 고정하는 테스트를 추가했다. 전체 단위 테스트 377개 통과.

## 2026-07-07 학년군 사이 cross-band edge 구축과 시각화 연결

- `build_cross_band_edges.py`를 추가해 초등·중학교·고1 미시 데이터셋 경계를 잇는 선수 관계 edge 16개를 `cross-band-edges.csv/.md`로 생성했다. 모든 edge는 별책8 원문이 앞 학년군 학습을 직접 지목하는 문장에만 근거한다: [9수01-02] 해설(초등 최대공약수·최소공배수→소인수분해 활용, 2개), 공통수학1 고려 사항(중학교 실수→복소수 2개, 다항식 곱셈·인수분해→고등 다항식 2개, 다항식÷단항식→조립제법, 연립일차방정식→연립부등식, 경우의 수→합·곱의 법칙 2개), 공통수학2 고려 사항·해설(일차함수 그래프·일차방정식 관계→두 직선의 평행·수직 조건 2개, 중학교 함수→두 집합 사이의 대응 함수). 과목 성격의 심화·체계화 서술 기반 3개(경우의 수→순열, 이차방정식/이차함수→이차방정식·이차함수의 관계)는 `medium`+메모로 구분했다.
- 빌더는 세 데이터셋을 로드해 id 존재를 검증하고, 낮은 학년군→높은 학년군 정방향만 허용하며, 역방향이나 미정의 id는 빌드 실패로 처리한다. `test_build_cross_band_edges.py` 7개로 id 유효성, 정방향, 근거 locator 필수, 신뢰도-근거 유형 일치, 대표 연결 존재를 고정했다.
- `build_k12_spine_viz.py`가 `cross-band-edges.csv`를 로드해 각 edge를 양쪽 블록에 붙이도록 확장했다(concept id→블록 매핑). `k12-spine.html` 패널에 세 번째 탭 `학년군 연결 N`이 생겨, 블록 선택 시 "앞 학년군에서 들어오는 연결"과 "다음 학년군으로 이어지는 연결"을 근거 문장·페이지와 함께 보여 준다. 예: 중1-3 수와 연산 블록은 in 2개(초5-6 약수·배수)와 out 2개(실수→복소수, 근호 계산→복소수 연산)를 표시한다.
- 브라우저에서 중1-3 수와 연산(in+out), 초5-6 수와 연산(out만), 공통수학2 함수와 그래프(in만)의 탭 표시와 근거 렌더를 실제 클릭으로 검증했다. SCHEMA.md에 `Cross Band Edges CSV` 스키마를 추가했다.
- 전체 단위 테스트 385개 통과, validator 977개 concept·4571개 edge 통과(기존 데이터 무변경).

## 2026-07-07 교과서 반입 점검(2022 개정 중1·중2)

- 사용자 다운로드 폴더에서 수학 교과서를 찾아, 2015 개정판과 2022 개정판이 섞여 있음을 확인했다. 우리 map은 2022 개정 기반이므로 2022 개정 완결본만 반입했다: 중1 5단원 + 중2 5단원, 총 10개 PDF. `15개정` 표기 교과서와 고등 선택 과목(인공지능·산업수학)은 제외했다.
- 판 식별은 본문의 "핵심 아이디어"(2022 개정 문구)와 학년군 표기 "초5~6"·"중1~3"(2022 개정 편제)로 했다. 이준열 교과서는 p.3에 "2015 개정 교육과정" 명시가 있어 제외 확정.
- 10개를 `교과서_원본/`에 `2022_중N_미확인_교과서_M단원.pdf` 규칙명으로 복사하고, SHA-256·크기·수집일을 `TEXTBOOK_SOURCE_MANIFEST.csv`에 기록했다. 출판사는 표지·판권면이 이미지라 텍스트로 확정 못 해 `미확인`으로 두었다(중1은 김화경 저자 콘텐츠와 일치하나 미확정).
- `build_textbook_source_audit.py`를 실행한 결과 10개 전부 PDF 헤더 정상·파일명 규칙 통과·매니페스트 연결됨이며, 공개 출처 URL이 없어 `intake_status = needs_manifest_metadata`이다. 즉 정식 추출 게이트는 사용자에게 출처 URL을 받기 전까지 닫혀 있다(설계대로).
- 저작권 규칙(개념명·쪽수만)에 따라 교과서 계통도를 확인한 결과, 중1 수와 연산 p.3 "배운 내용" 상자가 최대공약수·최소공배수를 "초5~6" 학습으로 명시해, cross-band edge(초5-6 공약수·공배수 → 중1 소인수분해 활용)를 교과서가 독립적으로 뒷받침함을 확인했다. 상세는 `textbook-intake-findings.md`.
- 교과서 PDF는 저작권·용량 문제로 `교과서_원본/.gitignore`(`*.pdf` 등)로 커밋에서 제외하고, 매니페스트·감사·findings만 버전 관리한다.
- 전체 단위 테스트 385개 통과(빌더 로직 무변경, 감사 산출물만 갱신).

## 다음 작업

- 교과서 정식 추출을 시작하려면 각 교과서의 공개 출처 URL(또는 사용 근거)을 `TEXTBOOK_SOURCE_MANIFEST.csv`의 `source_url`·`attachment_id`에 채워 감사가 `ready_for_textbook_extraction`이 되게 한다. 출판사 확정 후 파일명 `미확인`도 교체한다.
- 추출 시작 시 `textbook-evidence-workplan.*` 상위 rank 단원부터 개념명·짧은 정의·쪽수·관계 근거만 반영한다(원문 전재 금지). 초등·고1 데이터셋에도 같은 교과서 근거 패킷 체계를 적용한다.
- Scope Rules(AGENTS.md)에 따라 고등학교 선택 과목은 시각화·미시 분해·근거 보강에서 계속 제외한다.
- cross-band edge는 문서 직접 서술 기반 16개로 시작했다. 성취수준 문서·연구보고서에서 추가 연계 서술을 확인하면 같은 규칙으로 늘린다.
- 미추출 수식 기호(초1-2 +,−,=,<,>; 공통수학 공식 목록)는 시각 확인 또는 교과서 근거로 반영한다.
