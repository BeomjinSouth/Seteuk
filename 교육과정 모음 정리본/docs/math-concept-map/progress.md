...
- `related-edge-resolution-queue.csv`의 남은 low 34건과 backlog 2건을 공식 근거와 교과서 근거 기준으로 검토해 `represented_by`, `used_in`, `contrasts_with`, `related_to` edge 중 하나로 확정한다.
- 현재 queue 최상단은 미지수와 변수, 부등식의 해와 방정식의 해, 일차부등식과 연립일차방정식 단원, 일차함수와 연립일차방정식 단원, 함숫값과 식의 값, 일차함수·일차방정식 관계 단원과 좌표평면·그래프 단원 관련 pair이므로 다음 반복에서 우선 처리한다.
- 중1~중3 수학 교과서 PDF가 추가되면 `textbook-evidence-packets/`, `prerequisite-map.csv`, `prerequisite-unit-graph.dot`, `related-edge-resolution-queue.csv`를 함께 사용해 concept별 본문·예제 근거와 관계 edge별 쪽수 근거를 추적한다.
