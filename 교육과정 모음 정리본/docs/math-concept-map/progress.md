...
- `k12-spine.html`의 블록 드릴다운을 성취기준 목록에서 미시 concept 목록(초등 `elementary-*`, 중학교 `concepts.json`, 고1 `hs-common-*`)으로 확장한다.
- 학년군 사이 데이터셋 경계를 잇는 cross-dataset edge(예: 초5-6 약수·배수→중1 소인수분해, 중3 이차함수→공통수학1 이차방정식·이차함수 관계)를 문서의 직접 연계 서술 기반으로 추가하는 방법을 정한다.
