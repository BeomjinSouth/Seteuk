...
- `/api/generate`, `/settings/ai`, and `src/lib/write-logic.ts` share `SETEUK_DEFAULT_SYSTEM_PROMPT` from `src/lib/prompts/seteuk.ts`; the current prompt version is `cross-curricular-seteuk-v2`, which frames output as a teacher-review draft, prioritizes direct observations and student-specific evidence, and blocks unsupported facts, level/score wording, AI-tool wording, and unsupported 2026 기재요령 exclusion categories.
- `/settings/ai` exposes two prompt choices: `기본 설정` uses read-only `cross-curricular-seteuk-v2`, while `내 프롬프트` stores `seteukPromptMode` and `personalSeteukPrompt` in teacher workspace state and Supabase sync payloads.
- `src/lib/prompts/seteuk.ts` also exports `SETEUK_DEFAULT_EXAMPLE_TEMPLATE`; `src/lib/store.ts` and `/examples` share that neutral observation-based template, and stored legacy default templates are migrated to the v2 template without overwriting teacher-edited templates.
