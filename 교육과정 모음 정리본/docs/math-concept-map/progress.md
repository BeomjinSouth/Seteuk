...
- 재생성 결과 48개 row 분포는 `candidate_prerequisite_evidence` 3개, `candidate_assessment_item_evidence` 7개, `candidate_achievement_level_evidence` 15개, `broad_report_context_only` 22개, `weak_occurrence_only` 1개가 되었다.
- source ref 적용 상태는 `applied_after_manual_review` 3개, `pending_manual_review` 22개, `not_applicable_from_this_row` 23개로 정리되었다.
- `SCHEMA.md`, `README.md`, `source-audit.md`를 전체 page 텍스트 기반 source-review 분류와 최신 분포에 맞게 갱신했다.
- 좁은 생성기 테스트: `python -m unittest discover -s docs/math-concept-map/tools -p "test_build_research_report_source_review.py"`: 4개 통과.
- 전체 단위 테스트: `python -m unittest discover -s .\docs\math-concept-map\tools -p "test_*.py"`: 181개 통과.
- 전체 validator: `python .\docs\math-concept-map\tools\validate_concept_map.py`: 476개 concept, 1966개 edge, 5개 source, 60개 공식 성취기준 검증 통과.
- diff check: `git diff --check -- docs/math-concept-map 교과서_원본`: 종료 코드 0, CRLF 변환 경고만 확인.
- PDF 원본과 `2022_개정_중학교_교육과정_PDF/DOWNLOAD_MANIFEST.md`, `2022_개정_중학교_성취수준_PDF/DOWNLOAD_MANIFEST.md`는 변경하지 않았다.
...
- 교과서 PDF가 추가되기 전에는 `research-report-source-review.*`의 남은 `pending_manual_review` 22개를 검토해, 공식 연구보고서 맥락이 실제 concept 정의·예시 평가도구·채점 기준 근거인지 확인하고 source ref 또는 confidence 보강 여부를 별도로 결정한다. 단, `비` confidence 승격은 교과서 본문 또는 중학교 과정 직접 근거 확인 후 판단한다.
