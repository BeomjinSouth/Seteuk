...
async function prepareStudent(input: BatchStudentInput): Promise<PreparedStudent> {
    ...
    const observationBoardText = formatObservationBoardContextForPrompt(input.observationBoardContext);
...
function formatOcrContext(context?: OcrEvaluationContext): string {
    ...
    if (context.achievementStandards?.length) {
        lines.push('Achievement standards:');
        context.achievementStandards.forEach((standard) => {
            lines.push(`- ${standard.code} ${standard.description}`);
        });
    }

    if (context.scoringCriteria?.length) {
        lines.push('Scoring criteria:');
        context.scoringCriteria.forEach((criteria) => {
            lines.push(`- ${criteria.element}`);
        });
    }

    const result = context.studentResult;
    const feedbacks = result?.scores?.filter((score) => score.feedback?.trim()) || [];
    if (feedbacks.length > 0) {
        lines.push('Student feedback:');
        feedbacks.forEach((score) => {
            lines.push(`- ${score.criteriaElement}: ${score.feedback}`);
        });
    }
...
function buildBatchPrompt(students: PreparedStudent[], exampleTemplates: string[]): string {
    const studentBlocks = students.map((student, index) => {
        const variation = formatSeteukExpressionVariationForPrompt(
            resolveSeteukExpressionVariation({
        ...
        return [
            ...
            '',
            '[Curriculum context: background only, not student evidence]',
            student.curriculumContent?.trim() || '- none',
            '',
    ...
    const examples = exampleTemplates.length > 0
        ? `\n\n[Reference style examples]\n${exampleTemplates.join('\n\n')}`
        : '';

    return [
        'Write separate Korean 세특 drafts for the following students.',
        'Use only each student block as evidence for that student.',
        'Do not copy facts, attitudes, achievements, or wording from one student into another student.',
        ...
        'Return strict JSON only, with this shape: {"results":[{"studentId":"...","content":"..."}]}.',
...
export async function POST(request: NextRequest) {
    ...
    const fallbackResults = prepared
        .filter((student) => !hasDirectEvidence(student) && shouldUseSafeSeteukFallback(student.sanitizedLearningData, student.subjectName))
        .map((student) => ({
            studentId: student.studentId,
            studentName: student.studentName,
    ...
    if (!hasOpenAIApiKey() || openAiStudents.length === 0) {
        ...
        return NextResponse.json({
            success: true,
            batch: true,
            results: [...fallbackResults, ...noKeyFallbacks],
        });
    }
    ...
    try {
        ...
        const generatedResults = openAiStudents.map((student) => {
            ...
            return {
                studentId: student.studentId,
                studentName: student.studentName,
                content: sanitizeGeneratedSeteukContent(rawContent, student.sanitizedLearningData),
                observationCount: student.usedObservationIds.length,
                usedObservationIds: student.usedObservationIds,
                observationBoardContextCount: student.observationBoardContextCount,
