...
- `/write`: default seteuk generation prompt uses the `cross-curricular-seteuk-v2.9` teacher-review draft policy; it prioritizes direct teacher observations and student-specific evidence, treats curriculum content as background only, refuses to turn absent/generic/subject-only/contradictory evidence into fabricated 세특, calibrates length/detail by evidence amount, allows sparse but diligent students to be described concisely without exaggeration, keeps strong-student longer comments evidence-bound, rewrites low-level artifact-processing phrases into observable learning behaviors, avoids invented subject concepts, varies first phrase/focus/verb choices across multiple students, keeps cross-curricular large-batch guards against exact duplicate comments/internal repetition/length outliers, blocks unsupported achievement/attitude/competency/media/output-format claims, avoids unsupported quality wording such as `빠짐없이`, `체계적으로`, or `꾸준히`, and excludes score/rank/award/test-item/student-level/unsupported achievement/AI-tool wording
- `/api/generate`: learning data is sanitized before prompt construction and generated text is post-processed so exaggerated commands, inappropriate comparative requests, unsupported `과제 수행함`, unsupported `수정/보완/반영/조정` claims, raw 차시/횟수 wording, and generic curriculum-summary sentences do not leak into the final draft. The route keeps the existing JSON response contract, uses deterministic expression-variation guidance for batch generation, and returns the safe fallback sentence when usable evidence is absent.
- `/api/generate-batch`: selected `/write` students can be generated in one request so the shared system prompt, examples, and class context are sent once for the batch. Per-student safety fallbacks still apply, and `/write` falls back to single-student generation if the batch request fails.
- `/settings/ai`: teachers choose exactly one of `기본 설정` and `내 프롬프트`; the default is read-only `cross-curricular-seteuk-v2.9`, while the personal prompt is teacher-private and synced in workspace state
...
- AI settings stores admin overrides in browser storage, while empty or legacy default values resolve back to the shared `cross-curricular-seteuk-v2.9` default prompt; legacy default example templates are migrated to the neutral short-input example set while teacher-edited templates are preserved
