...
- Graph RAG counsel mode: `/counsel-chat` now includes a source-annotated Q&A tab with `/api/counsel-chat/graph`, answer-first citation annotations, highlighted answer spans only when a direct public source match exists, line-fragment highlighter rendering for wrapped answer/source text, compact citation markers, right-side source excerpt viewer, answer annotation list, answer font-size slider, collapsed current-question state after generation, and supplementary ontology/data-flow graph visualization
