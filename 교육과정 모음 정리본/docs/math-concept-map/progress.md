...
- 현재 교과서 원본 PDF가 없으므로, 추출 시작 단원은 `좌표평면과 그래프`의 concept 43개와 edge packet row 234개, 총 277개 row로 유지하되, rank 2 `일차함수와 그 그래프`의 concept 33개와 edge packet row 165개, 총 198개 row, rank 3 `경우의 수와 확률`의 concept 26개와 edge packet row 112개, 총 138개 row, rank 4 `이차함수와 그 그래프`의 concept 30개와 edge packet row 143개, 총 173개 row, rank 5 `정수와 유리수`의 concept 41개와 edge packet row 191개, 총 232개 row, rank 6 `도수분포표와 상대도수`의 concept 33개와 edge packet row 180개, 총 213개 row도 같은 방식으로 교과서 근거를 보강한다. 교과서 PDF 없이 공식·보조 문서 기반 미시 concept 보강을 계속한다면 다음 반복 후보는 rank 7 `문자의 사용과 식`과 rank 8 `일차방정식`이다.
