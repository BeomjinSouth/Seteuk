...
- `/write`: multi-student AI generation sends selected students to `/api/generate-batch` first so the shared prompt and class context are reused across the batch; if the batch route fails, the UI falls back to the existing single-student `/api/generate` path.
...
- `/write`: default seteuk generation prompt uses the `cross-curricular-seteuk-v2.5` teacher-review draft policy; it prioritizes direct teacher observations and student-specific evidence, treats observation-board activity as supporting attitude evidence only, uses selected unit context before legacy curriculum content and grade/subject hints, keeps context as lesson background rather than achievement evidence, removes raw count/frequency wording such as `1회`, avoids repeated `정리함/작성함/발표에 참여함` frames, varies sentence structure and subject-specific action verbs, prevents multi-unit list dumping, constrains activity-sheet/submission-only inputs to observable writing or recording behavior, and excludes score/rank/award/test-item/student-level/unsupported achievement/AI-tool wording
- `/settings/ai`: teachers choose exactly one of `기본 설정` and `내 프롬프트`; the default is read-only `cross-curricular-seteuk-v2.5`, while the personal prompt is teacher-private and synced in workspace state. Personal prompt text is not rewritten, but the global observation-board evidence minimization policy applies to both modes
- `/write`: default seteuk generation prompt uses the `cross-curricular-seteuk-v2.7` teacher-review draft policy; it prioritizes direct teacher observations and student-specific evidence, uses grade/subject/curriculum context only as background, varies first phrase/focus/verb choices across multiple-student generation batches, refuses to turn absent/generic/subject-only/contradictory evidence into fabricated 세특, blocks unsupported achievement/attitude/competency/media/output-format claims, avoids unsupported quality wording such as `빠짐없이`, `체계적으로`, or `꾸준히`, and excludes score/rank/award/test-item/student-level/unsupported achievement/AI-tool wording
- `/api/generate`: learning data is sanitized before prompt construction, a deterministic expression-variation profile is injected as private prompt guidance, and generated text is post-processed so exaggerated commands, inappropriate comparative requests, unsupported `과제 수행함`, unsupported `수정/보완/반영/조정` claims, raw 차시/count wording, repeated connectors, and generic curriculum-summary sentences do not leak into the final draft. The route keeps the existing JSON response contract and returns the safe fallback sentence when usable evidence is absent.
- `/settings/ai`: teachers choose exactly one of `기본 설정` and `내 프롬프트`; the default is read-only `cross-curricular-seteuk-v2.7`, while the personal prompt is teacher-private and synced in workspace state
...
- seteuk generation does not load student-data tab entries; it uses observation notes, learning data, OCR evaluation context, and a compact observation-board attitude signal derived from △/○ marks. Group numbers, roles, activity names, dates, session topics, and raw marks are removed before both single and batch generation
- AI settings stores admin overrides in browser storage, while empty or legacy default values resolve back to the shared `cross-curricular-seteuk-v2.5` default prompt; legacy default example templates are migrated to the neutral short-input example set while teacher-edited templates are preserved
...
- AI settings stores admin overrides in browser storage, while empty or legacy default values resolve back to the shared `cross-curricular-seteuk-v2.7` default prompt; legacy default example templates are migrated to the neutral short-input example set while teacher-edited templates are preserved
...
- student observation sessions: each selected class keeps its own session headers; new classes open with blank date/topic fields instead of demo activities unless a reviewed class schedule is bundled, such as the 3학년 1반 수학 activity sequence. For 3학년 1반, stale saved blank/default or unrelated reading/book activity headers are normalized back to the reviewed math sequence, while teacher-entered non-reading custom headers remain preserved. Teachers can enter class-specific dates and activity content, the `+` header button adds another session column only for that class, and teacher-clicked student △/○ marks are converted into attitude/participation summaries for `/write` generation
...
- student observation teacher scope: session headers, activity marks, mentor assignments, and marked-session snapshots are stored under the logged-in teacher and selected class, so teacher A and teacher B can keep different records for the same 3학년 1반 roster
- student observation data safety: the board keeps rolling local backups, exposes manual backup/restore in settings, and blocks suspicious automatic syncs that would suddenly drop established session headers, activity marks, or mentor assignments to zero unless the teacher explicitly chose a reset or restore action
