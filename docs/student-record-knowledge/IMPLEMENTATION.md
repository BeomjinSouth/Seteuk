...
- `/api/generate-batch` accepts selected students plus shared prompt/model/example context and sends eligible students in one OpenAI Responses call, so the system prompt and shared class guidance are not repeated for every student. Per-student safety fallbacks still return without OpenAI, and `/write` falls back to the single-student route if the batch call fails.
...
- `/observation-board-2` stores editable session headers as `classId -> sessions` under `observation-board-2-sessions:${teacherKey}`; new class headers start with blank date/topic fields unless `getDefaultObservationBoardActivitySessionsForClass` has a reviewed class schedule such as 3학년 1반 수학, legacy demo headers such as `5/2 (금)` and `서로 알아가기` are normalized to blank fields on load, and 3학년 1반 saved headers that are blank/default or contain unrelated reading/book keywords are normalized back to the reviewed math sequence in both the board UI and `/write` AI context. Teachers edit session date/content inline, add columns with the `+` button for only the selected class, and legacy teacher-wide session arrays are migrated to the first assigned class for compatibility
...
- Supabase runtime storage is supported through `src/lib/supabase/*`; production requires `SUPABASE_URL`/`SUPABASE_PROJECT_ID` plus `SUPABASE_SECRET_KEY`. Sheet reads normally use Supabase, but `src/lib/sheets/base.ts` can fall back to Google Sheets reads when a configured Supabase host fails with a network/DNS error and Google Sheets credentials are present. Mutations remain Supabase-only in production except for the append-only cookie ledger: cookie transactions use an atomic row append, fall back to a Google Sheets append only for confirmed Supabase network/DNS failures, and merge both ledgers by transaction id on reads so outage-time records remain visible after recovery.
...
- Existing Google Sheets data can be migrated with `scripts/migrate-google-sheets-to-supabase.mjs` after applying `supabase/migrations/202604270001_initial_seteuk_storage.sql`; Google Sheets is otherwise retained for local dev/import/migration and the read-only production failover above only.
...
- `/write` reads the teacher-scoped, class-specific observation-board session headers, student △/○ marks, current mentor/mentee assignments, and per-session assignment snapshots from localStorage and sends a derived `observationBoardContext`. Before prompt assembly, `formatObservationBoardContextForPrompt` removes group numbers, role names, activity names, dates, session topics, and raw mark details, leaving only a compact participation-attitude signal and usage limits
- `/api/generate`, `/settings/ai`, and `src/lib/write-logic.ts` share `SETEUK_DEFAULT_SYSTEM_PROMPT` from `src/lib/prompts/seteuk.ts`; the current prompt version is `cross-curricular-seteuk-v2.5`, which frames output as a teacher-review draft, prioritizes direct observations and student-specific evidence, limits observation-board activity to short supporting attitude evidence, applies selected unit context before legacy curriculum content and grade/subject hints, treats context as lesson background rather than student achievement evidence, removes count/frequency wording such as `1회`, avoids repeated `정리함/작성함/발표에 참여함` sentence frames, prevents multi-unit list dumping, constrains activity-sheet/submission-only inputs to observable writing or recording behavior, and blocks unsupported facts, level/score wording, AI-tool wording, and unsupported 2026 기재요령 exclusion categories.
- `/api/generate` accepts optional `gradeLevel` from `/write`; `src/lib/write-logic.ts` sends the student's grade along with subject, learning data, curriculum content, observation-board context, OCR context, and semester. When curriculum content is blank, the route adds a short grade/subject hint such as broad math, Korean, science, social studies, English, arts, PE, technology/home economics, information, or ethics topics so sparse inputs do not collapse into generic participation wording.
- `/write` uses `/api/generate-batch` for multi-student AI generation while keeping the existing single-student route for fallback and compatibility; both routes call the same observation-board formatter with the student's observation memo and meaningful learning-data presence so the evidence-weight rule cannot diverge.
- `/settings/ai` exposes two prompt choices: `기본 설정` uses read-only `cross-curricular-seteuk-v2.5`, while `내 프롬프트` stores `seteukPromptMode` and `personalSeteukPrompt` in teacher workspace state and Supabase sync payloads. Personal prompt text remains unchanged, while global observation-board evidence minimization is enforced in prompt context assembly for both modes.
- `src/lib/prompts/seteuk.ts` also exports `SETEUK_DEFAULT_EXAMPLE_TEMPLATE`; `src/lib/store.ts` and `/examples` share that neutral short-input example set across different subjects, and stored legacy/default templates are migrated to the current neutral template without overwriting teacher-edited templates.
...
- `/api/generate`, `/settings/ai`, and `src/lib/write-logic.ts` share `SETEUK_DEFAULT_SYSTEM_PROMPT` from `src/lib/prompts/seteuk.ts`; the current prompt version is `cross-curricular-seteuk-v2.7`, which frames output as a teacher-review draft, prioritizes direct observations and student-specific evidence, uses grade/subject/curriculum context as background, varies first phrase/focus/verb choices for multiple-student generation batches, blocks unsupported achievement/attitude/competency/media/output-format claims, prevents `친구 의견을 들음` from becoming `수정/보완/반영`, and avoids unsupported quality wording such as `빠짐없이`, `체계적으로`, `꼼꼼히`, `충실히`, `꾸준히`, or `지속적으로`.
- `/api/generate` accepts optional `gradeLevel` and `curriculumContext` from `/write`; `src/lib/write-logic.ts` sends the student's grade along with subject, learning data, curriculum content, observation-board context, OCR context, and semester. When curriculum content is blank, the route still adds a short grade/subject hint so sparse but concrete inputs do not collapse into generic participation wording.
...
- `/api/generate` removes exaggerated/non-evidence request fragments such as score, rank, award, future prediction, personality praise, direct writing commands, and inappropriate comparative requests before sending the prompt. It also adds a deterministic expression-variation prompt block from `src/lib/seteuk-expression-variation.ts` so similar student inputs do not all use the same opening frame or verb set. Generated content is post-processed to neutralize unsupported quality words, unsupported `과제 수행함`, unsupported revision actions such as `수정함/보완함/반영함/조정함`, raw 차시/count wording, awkward `수업 태도에 맞게`, repeated `이를 바탕으로`, and generic curriculum-summary sentences when absent from the source evidence while keeping the existing JSON response shape.
- `/settings/ai` exposes two prompt choices: `기본 설정` uses read-only `cross-curricular-seteuk-v2.7`, while `내 프롬프트` stores `seteukPromptMode` and `personalSeteukPrompt` in teacher workspace state and Supabase sync payloads.
- `src/lib/prompts/seteuk.ts` also exports `SETEUK_DEFAULT_EXAMPLE_TEMPLATE`; `src/lib/store.ts` and `/examples` share that neutral short-input example set across different subjects, and stored legacy/default templates are migrated to the v2.7 template without overwriting teacher-edited templates.
