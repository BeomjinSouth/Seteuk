...
검증기는 필수 필드, id 중복, source/ref 무결성, CSV 행 수, Mermaid 파일, 공식 성취기준 60개(`9수01-01`~`9수04-09`)의 concept 근거 커버리지, `review-queue.csv`와 `low` 신뢰도 concept 수 196개의 일치, 공식 용어·기호 168개 커버리지, concept alias 감사 row 996개, 연구보고서 concept 출현 후보 276개, 연구보고서 context packet 67개 row와 source-review 67개 row, `related-edge-resolution-queue.csv` 0개, `node-edge-consistency-audit.csv` 0개, `prerequisite-map.csv`의 `prerequisite_for` edge 1652개, 고립 concept 0개, concept 966개와 edge 4505개의 근거 깊이, 교과서 PDF 부재 상태의 concept evidence row 966개와 단원 접점 edge row 5314개, workplan pending 6280개, 전체 unit-map concept 966개와 단원 접점 edge row 5314개를 재생성 결과와 일치하게 보존하는지 확인한다.
