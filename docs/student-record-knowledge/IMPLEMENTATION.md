...
- `/api/generate`, `/settings/ai`, and `src/lib/write-logic.ts` share `SETEUK_DEFAULT_SYSTEM_PROMPT` from `src/lib/prompts/seteuk.ts`; the current prompt version is `cross-curricular-seteuk-v2.4`, which frames output as a teacher-review draft, prioritizes direct observations and student-specific evidence, applies selected unit context before legacy curriculum content and grade/subject hints, treats context as lesson background rather than student achievement evidence, removes count/frequency wording such as `1회`, avoids repeated `정리함/작성함/발표에 참여함` sentence frames, prevents multi-unit list dumping, constrains activity-sheet/submission-only inputs to observable writing or recording behavior, and blocks unsupported facts, level/score wording, AI-tool wording, and unsupported 2026 기재요령 exclusion categories.
...
- `/write` uses `/api/generate-batch` for multi-student AI generation while keeping the existing single-student route for fallback and compatibility.
- `/settings/ai` exposes two prompt choices: `기본 설정` uses read-only `cross-curricular-seteuk-v2.4`, while `내 프롬프트` stores `seteukPromptMode` and `personalSeteukPrompt` in teacher workspace state and Supabase sync payloads.
- `src/lib/prompts/seteuk.ts` also exports `SETEUK_DEFAULT_EXAMPLE_TEMPLATE`; `src/lib/store.ts` and `/examples` share that neutral short-input example set across different subjects, and stored legacy/default templates are migrated to the v2.4 template without overwriting teacher-edited templates.
