...
- `/api/generate`, `/settings/ai`, and `src/lib/write-logic.ts` share `SETEUK_DEFAULT_SYSTEM_PROMPT` from `src/lib/prompts/seteuk.ts`; the current prompt version is `cross-curricular-seteuk-v2.3`, which frames output as a teacher-review draft, prioritizes direct observations and student-specific evidence, uses grade/subject/curriculum context to choose a plausible subject background for short inputs even when no unit name is entered, removes count/frequency wording such as `1회`, avoids repeated `정리함/작성함/발표에 참여함` sentence frames, and blocks unsupported facts, level/score wording, AI-tool wording, and unsupported 2026 기재요령 exclusion categories.
...
- `/settings/ai` exposes two prompt choices: `기본 설정` uses read-only `cross-curricular-seteuk-v2.3`, while `내 프롬프트` stores `seteukPromptMode` and `personalSeteukPrompt` in teacher workspace state and Supabase sync payloads.
- `src/lib/prompts/seteuk.ts` also exports `SETEUK_DEFAULT_EXAMPLE_TEMPLATE`; `src/lib/store.ts` and `/examples` share that neutral short-input example set across different subjects, and stored legacy/default templates are migrated to the v2.3 template without overwriting teacher-edited templates.
