...
- `/write`: default seteuk generation prompt uses the `cross-curricular-seteuk-v2.4` teacher-review draft policy; it prioritizes direct teacher observations and student-specific evidence, uses selected unit context before legacy curriculum content and grade/subject hints, keeps context as lesson background rather than achievement evidence, removes raw count/frequency wording such as `1회`, avoids repeated `정리함/작성함/발표에 참여함` frames, varies sentence structure and subject-specific action verbs, prevents multi-unit list dumping, constrains activity-sheet/submission-only inputs to observable writing or recording behavior, and excludes score/rank/award/test-item/student-level/unsupported achievement/AI-tool wording
- `/settings/ai`: teachers choose exactly one of `기본 설정` and `내 프롬프트`; the default is read-only `cross-curricular-seteuk-v2.4`, while the personal prompt is teacher-private and synced in workspace state
...
- AI settings stores admin overrides in browser storage, while empty or legacy default values resolve back to the shared `cross-curricular-seteuk-v2.4` default prompt; legacy default example templates are migrated to the neutral short-input example set while teacher-edited templates are preserved
