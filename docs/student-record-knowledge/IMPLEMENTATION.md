...
- `/api/generate`, `/settings/ai`, and `src/lib/write-logic.ts` share `SETEUK_DEFAULT_SYSTEM_PROMPT` from `src/lib/prompts/seteuk.ts`; the current prompt version is `cross-curricular-seteuk-v2.7`, which frames output as a teacher-review draft, prioritizes direct observations and student-specific evidence, uses grade/subject/curriculum context as background, blocks unsupported achievement/attitude/competency/media/output-format claims, prevents `친구 의견을 들음` from becoming `수정/보완/반영`, and avoids unsupported quality wording such as `빠짐없이`, `체계적으로`, `꼼꼼히`, `충실히`, `꾸준히`, or `지속적으로`.
