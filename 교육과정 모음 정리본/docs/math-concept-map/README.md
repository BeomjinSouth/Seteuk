...
검증기는 필수 필드, id 중복, source/ref 무결성, CSV 행 수, Mermaid 파일, 공식 성취기준 60개(`9수01-01`~`9수04-09`)의 concept 근거 커버리지, `review-queue.csv`와 `low` 신뢰도 concept 수 81개의 일치, 공식 용어·기호 168개 커버리지, concept alias 감사 row 563개와 `equivalent_to` edge 3개, 연구보고서 concept 출현 후보 242개, 연구보고서 context/source-review 53개 row, `related-edge-resolution-queue.csv` 0개, `prerequisite-map.csv`의 `prerequisite_for` edge 829개, 고립 concept 0개, concept 540개와 edge 2319개의 근거 깊이, 교과서 PDF 부재 상태의 concept 패킷 540개와 edge 패킷 2792개 row, workplan pending 3332개, rank 1 `좌표평면과 그래프` concept 43개와 edge 237개, rank 4 `이차함수와 그 그래프` concept 30개와 edge 143개, rank 6 `도수분포표와 상대도수` concept 33개와 edge 180개, rank 7 `일차방정식` concept 31개와 edge 159개, rank 8 `문자의 사용과 식` concept 24개와 edge 143개, rank 9 `다항식의 곱셈과 인수분해` concept 20개와 edge 114개, 전체 unit-map concept 540개와 단원 접점 edge row 2792개를 재생성 결과와 일치하게 보존하는지 확인한다.
